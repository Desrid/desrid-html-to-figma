...
if (!['codex', 'claude', 'both'].includes(agent)) fail('--agent must be codex, claude, or both');
if (!['project', 'user'].includes(scope)) fail('--scope must be project or user');

const base = path.resolve(args.target || (scope === 'user' ? os.homedir() : process.cwd()));
...
if (agent === 'codex' || agent === 'both') {
  destinations.push({ agent: 'codex', path: path.join(base, '.agents', 'skills', SKILL_NAME) });
}
if (agent === 'claude' || agent === 'both') {
  destinations.push({ agent: 'claude', path: path.join(base, '.claude', 'skills', SKILL_NAME) });
}

const results = [];
for (const destination of destinations) {
  results.push(args.check ? await checkDestination(destination) : await installDestination(destination));
}
...
async function installDestination(destination) {
  ...
  if (exists) {
    ...
    const locallyModified = !currentMarker || currentHash !== currentMarker.sourceHash;
  ...
  const temp = path.join(parent, `.${SKILL_NAME}.tmp-${process.pid}-${Date.now()}`);
  await fs.mkdir(parent, { recursive: true });
  await fs.rm(temp, { recursive: true, force: true });
  await fs.cp(sourceRoot, temp, { recursive: true, force: true });
  await fs.writeFile(path.join(temp, MARKER), `${JSON.stringify({
    name: SKILL_NAME,
    version,
  ...
  if (installedHash !== sourceHash) {
    return { ...destination, status: 'error', reason: 'Post-install content hash mismatch', installedHash };
  }
  return { ...destination, status: exists ? 'updated' : 'installed', hash: installedHash };
}
...
async function checkDestination(destination) {
  ...
  const installedHash = await hashTree(dest);
  if (!marker) {
    return { ...destination, status: 'drifted', reason: 'Missing installation marker', installedHash };
  }
  ...
  return { ...destination, status: 'current', hash: installedHash, version: marker.version };
}
...
async function hashTree(dir) {
  const files = await walkFiles(dir, { ignore: ['node_modules', '.git'], maxBytes: 20_000_000 });
