...
### Fixed
- Generic MCP clients now install from a versioned GitHub Release tarball, avoiding npm's slow Git repository preparation path while keeping distribution on GitHub.
- GitHub Releases now publish both the Claude Desktop `.dxt` package and the generic MCP `.tgz` package.

## [1.1.1] - 2026-08-24

### Fixed
- Git-backed `npx` installs now run the committed standalone MCP bundle directly, without rebuilding the project or downloading runtime dependencies before the MCP handshake.
